...
class CalendarHeatmap extends React.Component {
  ...
  cleanPreviousObjects()
  {
    ...
    //titlesel.select("#TitleLabel").remove();
    //titlesel.select("#DatesBound").remove();  
    titlesel.remove();
  }

  createAddons() // RGB: changing component to allow center title an other information related to the visualization
  {
    
    var titlesel = d3.select('#calendar-heatmap').select("svg");
    //var newsvg = titlesel.append("svg");
    var element = titlesel.node();
    var posx = element.getBoundingClientRect().width;
    var posy = element.getBoundingClientRect().height;
   // console.log(posy);
    var sdate = moment(this.props.startdate).format('MMMM Do YYYY');
    var edate = moment(this.props.enddate).format('MMMM Do YYYY');
    
    titlesel.append("text").text("From : "+sdate +"     to : "+edate).attr("transform", "translate(" + (posx/2) + "," + (posy - 30) + ")")
    .attr("text-anchor", "middle")
    .attr("font-size", this.props.font_size_date)
    .attr("id","DatesBound")
    .attr("fill", "#3a4245")
    .attr("class", "looker-vis-context-value-text");

    titlesel.append("text").text(this.props.title).attr("transform", "translate(" + (posx/2) + "," + (posy - 80) + ")")
    .attr("text-anchor", "middle")
    .attr("font-size", this.props.font_size_title)
    .attr("id","TitleLabel")
  ...
  calcDimensions() {
    ...
    this.svg.attr('width', this.settings.width)
      .attr('height', this.settings.height + 150) // RGB: adding extra space for titles and additional labels
  ...
  drawGlobalOverview() {
    ...
    let item_block = this.items.selectAll('.item-block-year')
      ...
      .on('mouseover', d => {
        ...
        // Construct tooltip
        let tooltip_html = ''
        tooltip_html += '<div><span><strong>Total:  </strong></span>'
        tooltip_html += '<span>' + d.total + '</span>';
        tooltip_html += '<span>   ' + this.props.totmeasure + '</span></div>';
        ...
        if (d.summary.length <= 5) {
          ...
          while ( counter < d.summary.length ) {
           ...
           // tooltip_html += '<span>' + this.formatTime(d.summary[counter].value) + '</span></div>'
            tooltip_html += '<span>    ' + d.summary[counter].value + '</span>'
            tooltip_html += '<span>    ' + this.props.measure + '</span></div>'
            counter++
          }
        } else {
          let counter = 0
          ...
          tooltip_html += '<span>    ' + other_projects_sum + '</span>'
          tooltip_html += '<span>    ' + this.props.measure + '</span></div>';
        }
  ...
  drawYearOverview() {
    ...
    this.items.selectAll('.item-circle')
      ...
      .on('mouseover', d => {
        ...
        //RGB: Changes on the tooltip (hint) displayed in the monthly - day view
        // Construct tooltip
        let tooltip_html = ''
        //tooltip_html += `<div class="${styles.header}"><strong>${d.total ? this.formatTime(d.total) : 'No time'} tracked</strong></div>`
        tooltip_html += `<div class="${styles.header}"><strong>${d.total ? d.total : 'No entries for '} </strong>`
        tooltip_html += '<span><strong> Total  ' + this.props.measure + '</strong></span></div><br>'
        ...
        while ( counter < d.summary.length ) {
          ...
          //tooltip_html += '<span>' + this.formatTime(d.summary[counter].value) + '</span></div>'
          tooltip_html += '<span>    ' + d.summary[counter].value+ '</span>'
          tooltip_html += '<span><strong>    ' + this.props.measure + '</strong></span></div>'
          counter++
        }
  ...
  drawMonthOverview() {
    ...
    item_block.selectAll('.item-block-rect')
      ...
      .on('mouseover', d => {
        ...
        //RGB: Changes on the label to use the props parameter 
        // Construct tooltip
        let tooltip_html = ''
       ...
       // tooltip_html += '<div><strong>' + (d.value ? this.formatTime(d.value) : 'No time') + ' tracked</strong></div>'
        tooltip_html += '<div><strong>' + (d.value ? d.value : 'No entries for')+'   '+this.props.measure+' </strong></div>'
        //tooltip_html += '<span><strong> Total  ' + this.props.measure + '</strong></span></div>'
  ...
  drawWeekOverview() {
    ...
    item_block.selectAll('.item-block-rect')
      ...
      .on('mouseover', d => {
        ...
        // RGB: Changes on tooltip to add the props parameters
        // Construct tooltip
        let tooltip_html = ''
        tooltip_html += `<div class="${styles.header}"><strong>${d.name}</strong></div><br>`
        //tooltip_html += '<div><strong>' + (d.value ? this.formatTime(d.value) : 'No time') + ' tracked</strong></div>'
  ...
  drawDayOverview() {
    ...
    this.items.selectAll('.item-block')
      ...
      .attr('width', d => {
        ...
        return Math.max((end - itemScale(moment(d.date))), this.props.sizeonday)
        //RGB: adding the props parameter to redifine the size of the rectangle in the day view
      })
      .attr('height', () => {
  ...
  drawButton() {
    ...
    let button = this.buttons.append('g')
      ...
      .on('mouseover', d => {
        let tooltip_html = ''
        tooltip_html += `<div class="${styles.header}"><strong> Change View </strong></div><br>`
    ...
    button.append('text')
      ...
      .attr('x', this.settings.label_padding / 2)
      .attr('y', this.settings.label_padding / 2.5)
      .attr('dy', () => {
        return Math.floor(this.settings.width / 100) / 2.5
      })
      .attr('font-size', () => {
  ...
  render() {
     ...
     return (
          ...
          <CalendarChartWrapper id='calendar-heatmap'
